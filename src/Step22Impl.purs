...
type MyShows p
  = TypeclassCons' (TypePairC' (PProxy Z) ShowMe) (TypeclassCons' (TypePairC' (PProxy (Succ p)) ShowMe) (TypeclassCons' (TypePairC' Boolean ShowMe) TypeclassNil'))
...
--showPeano s = show s
-- doesn't know what it has to be going backwards
-- but it DOES know what it has to be going forwards!
-- somehow, we need to flip mathematical induction
-- or find a way to get this out of the typeclass system
showPeanoSucc' :: PProxy (Succ Z) -> String
showPeanoSucc' = show

showPeanoSucc'' _ = ""

showPeanoZ' :: PProxy Z -> String
showPeanoZ' = show

myShows :: forall p. Typeclass (MyShows p)
myShows = cons (Proxy :: Proxy (TypePairC' (PProxy Z) ShowMe)) (Proxy :: Proxy (PProxy Z)) (ShowMe $ showPeanoZ') empty (cons (Proxy :: Proxy (TypePairC' (PProxy (Succ p)) ShowMe)) (Proxy :: Proxy (PProxy (Succ p))) (ShowMe $ showPeanoSucc'') empty (cons (Proxy :: Proxy (TypePairC' Boolean ShowMe)) (Proxy :: Proxy Boolean) (ShowMe $ const "Fooled you with a fake boolean!") empty empty))
